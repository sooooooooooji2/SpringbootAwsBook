...
import com.springbootaws.book.config.auth.dto.OAuthAttributes;
import com.springbootaws.book.config.auth.dto.SessionUser;
import com.springbootaws.book.domain.user.User;
import com.springbootaws.book.domain.user.UserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        String userNameAttributeName = userRequest.getClientRegistration().getProviderDetails()
                .getUserInfoEndpoint().getUserNameAttributeName();      // 로그인 진행 시 키가 되는 필드
